...
class Model(torch.nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, num_layers, gnn_arch, node_classification_mode=False):
        super().__init__()
        self.dropout = dropout
        self.num_layer = num_layers
        self.gnn_arch = gnn_arch
        self.node_classification_mode = node_classification_mode
    ...
    def embed(self, x, edge_index, edge_weight, batch=None, **kwargs):
        ...
        # Conditional pooling: only pool for graph classification
        if batch is not None and not self.node_classification_mode:
            x = global_mean_pool(x, batch)
        # else: Node classification - keep all node embeddings
        return x
